...
{
	public class CameraDragDraw : MonoBehaviour
	{
		...
		public GameManagerDraw gameManager;//the game manager reference
		...
		void Start ()
		{
			...
			if (gameManager == null) {
				gameManager = GameObject.FindObjectOfType<GameManagerDraw> ();
			}
			maxSpeed = speed;
		...
		void Update ()
		{
			if (!GameManagerDraw.pointerInDrawArea) {
				return;
			}
		...
		void LateUpdate ()
		{
			...
			difference = initialOrthographicSize - CameraZoomDraw.currentOrthographicSize;
			if (difference > 0) {
		...
		{
			if (Input.touchCount != 1 || CameraZoomDraw.isCameraZooming) {
				return;
			}
			...
			if (touch.phase == TouchPhase.Began) {
				...
				if (CameraZoomDraw.zoomStartedBefore) {
					CameraZoomDraw.zoomStartedBefore = false;
					TouchBegan (touch.position);
					return;
				}
				...
				if (!CameraZoomDraw.zoomStartedBefore && touchBegan) {
					TouchRelease ();
				}
			}
		}
