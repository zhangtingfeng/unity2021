...
namespace IndieStudio.DrawingAndColoring.Logic
{
	...
	public class ShapesTableDraw : MonoBehaviour
	{
			/// <summary>
			...
			/// The shapes list.
			/// </summary>
			public static List<TableShapeDraw> shapes;
			...
			void Start ()
			{
					...
					shapes = new List<TableShapeDraw> ();
					...
					if (saveLastSelectedGroup && ShapesManagerDraw.instance!=null) {
						scrollSlider.currentGroupIndex = ShapesManagerDraw.instance.lastSelectedShape;
					}
			}
			...
			{
					if (ShapesManagerDraw.instance == null) {
						return;
					}
					...
					for (int i = 0; i < ShapesManagerDraw.instance.shapes.Count; i++) {
							...
							TableShapeDraw tableShapeComponent = tableShapeGameObject.GetComponent<TableShapeDraw> ();//get TableShape Component
							...
							GameObject content = Instantiate(ShapesManagerDraw.instance.shapes[i].gamePrefab,Vector3.zero,Quaternion.identity) as GameObject;
							...
							//add click listener
							tableShapeGameObject.GetComponent<Button> ().onClick.AddListener (() => GameObject.FindObjectOfType<UIEvents> ().AlbumShapeEvent (tableShapeGameObject.GetComponent<TableShapeDraw> ()));
							content.gameObject.SetActive (true);
							shapes.Add (tableShapeComponent);//add table shape component to the list
					}

					if (ShapesManagerDraw.instance.shapes.Count == 0) {
			...
			public void OnChangeGroup (int currentGroup)
			{
				...
				ShapesManagerDraw.instance.lastSelectedShape = currentGroup;
					}
			}
	}
}