...
namespace IndieStudio.DrawingAndColoring.Logic
{
	...
	public class GameManagerDraw : MonoBehaviour
	{
		/// <summary>
		/// The lines counter.
		/// </summary>
		...
		void OnDestroy() {
			...
			foreach (ShapesManagerDraw.Shape shape in ShapesManagerDraw.instance.shapes) {
				if(shape !=null)
		...
		private void LineFeatureOnClickBegan(){
			...
			line.transform.SetParent(Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].transform);
			...
			Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder++;
			if(Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder <= Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].lastPartSortingOrder){
				Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder = Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].lastPartSortingOrder+1;
			}
			...
			element.sortingOrder = Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder;
			Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].GetComponent<History> ().AddToPool(element);

			//Set the sorting order of the line (last line must be on the top)
			currentLine.SetSortingOrder(Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder);
			...
			if (currentLine != null) {
				...
				if (currentLine.GetPointsCount () == 0) {//Zero Points
					...
					if (!currentTool.roundedEdges) {
						//Destroy the line
						Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder--;
						Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].GetComponent<History> ().RemoveLastElement ();
						Destroy (currentLine.gameObject);
					} else {
		...
		private void StampFeatureOnClickBegan(){
			...
			stamp.transform.SetParent(Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].transform);
			...
			if (sr != null) {
				if(currentTool.audioClip!=null)
					CommonUtilDraw.PlayOneShotClipAt(currentTool.audioClip,Vector3.zero,1);//play Tool audio clip
				...
				sr.sortingOrder = Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder;
			}
		}
			...
			if (hit2d.collider != null) {
				...
				if(shapePart!=null){//Shape Part
					...
					if (spriteRenderer != null) {
						...
						History.Element lastElement = Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].GetComponent<History> ().GetLastElement();
						...
						if(shapePart.targetColor !=currentToolContent.gradientColor.colorKeys[0].color || !equalsLastElement){
							...
							Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].shapePartsColors [hit2d.collider.name] = currentToolContent.gradientColor;
							spriteRenderer.sortingOrder = Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder + 1;
							Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].shapePartsSortingOrder [hit2d.collider.name] = spriteRenderer.sortingOrder;
							Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].lastPartSortingOrder = spriteRenderer.sortingOrder;
		...
		private void InstantiateDrawingContents(){
			...
			if (Area.shapesDrawingContents.Count == 0 && ShapesManagerDraw.instance.shapes.Count!= 0) {
				foreach (ShapesManagerDraw.Shape sShape in ShapesManagerDraw.instance.shapes) {
					if(sShape == null){
						continue;
					}
					GameObject drawingContents = new GameObject (sShape.gamePrefab.name + " Contents");
					int intNameToLayer = LayerMask.NameToLayer("MiddleCamera");
					Debug.Log("intNameToLayer=" + intNameToLayer);
					drawingContents.layer = intNameToLayer;
					...
					Transform shapeParts = sShape.gamePrefab.transform.Find ("Parts");
					if (shapeParts != null) {
		...
		public void SetShapeOrder(){
			...
			if (ShapesManagerDraw.instance.shapes == null || ShapesCanvas.shapeOrder == null) {
				return;
			}

			ShapesCanvas.shapeOrder.text = (ShapesManagerDraw.instance.lastSelectedShape + 1)+"/"+ ShapesManagerDraw.instance.shapes.Count;
		}
		...
		public void LoadCurrentShape(){
			...
			if (ShapesManagerDraw.instance.shapes == null) {
				return;
			}

			if (!(ShapesManagerDraw.instance.lastSelectedShape >= 0 && ShapesManagerDraw.instance.lastSelectedShape < ShapesManagerDraw.instance.shapes.Count)) {
				return;
			}

			SetShapeOrder ();
			SetShapeOrderColor ();
			ShapesManagerDraw.instance.shapes [ShapesManagerDraw.instance.lastSelectedShape].gamePrefab.SetActive(true);
			Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].gameObject.SetActive (true);
			Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].GetComponent<History> ().CheckUnDoRedoButtonsStatus ();
		}
		...
		public void NextShape(){
			...
			ShapesManagerDraw.instance.shapes [ShapesManagerDraw.instance.lastSelectedShape].gamePrefab.SetActive(false);
			Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].gameObject.SetActive (false);

			if (ShapesManagerDraw.instance.lastSelectedShape + 1 >= ShapesManagerDraw.instance.shapes.Count) {
				ShapesManagerDraw.instance.lastSelectedShape = 0;
			} else {
				ShapesManagerDraw.instance.lastSelectedShape += 1;
			}
		...
		public void PreviousShape(){
			...
			if (ShapesManagerDraw.instance.lastSelectedShape - 1 < 0) {
				ShapesManagerDraw.instance.lastSelectedShape = ShapesManagerDraw.instance.shapes.Count - 1;
			} else {
				ShapesManagerDraw.instance.lastSelectedShape -= 1;
			}
		...
		public void CleanCurrentShapeScreen(){
			...
			if (Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape] == null) {
				return;
			}

			//Clean the history for the current shape
			Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].GetComponent<History> ().CleanPool ();

			//Remove all the childern in drawContents
			foreach (Transform child in Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].transform) {
				Destroy(child.gameObject);
			}

			Transform shapeParts = ShapesManagerDraw.instance.shapes [ShapesManagerDraw.instance.lastSelectedShape].gamePrefab.transform.Find ("Parts");
			if (shapeParts != null) {
				foreach(Transform part in shapeParts){
					part.GetComponent<SpriteRenderer>().color = Color.white;
					Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].shapePartsColors[part.name] = Color.white;
					part.GetComponent<ShapePart>().ApplyInitialSortingOrder();
					part.GetComponent<ShapePart>().ApplyInitialColor();
					Area.shapesDrawingContents [ShapesManagerDraw.instance.lastSelectedShape].shapePartsSortingOrder[part.name] = part.GetComponent<ShapePart>().initialSortingOrder;
				}
			}

			linesCount = 0;
			Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].currentSortingOrder = 0;
			Area.shapesDrawingContents[ShapesManagerDraw.instance.lastSelectedShape].lastPartSortingOrder = 0;
		}
		...
		public void CleanShapes(){
			...
			for (int i = 0; i < ShapesManagerDraw.instance.shapes.Count; i++) {
				//Clean the history for the current shape
				...
				Transform shapeParts = ShapesManagerDraw.instance.shapes [i].gamePrefab.transform.Find ("Parts");
				if (shapeParts != null) {
					foreach (Transform part in shapeParts) {
						part.GetComponent<SpriteRenderer> ().color = Color.white;
