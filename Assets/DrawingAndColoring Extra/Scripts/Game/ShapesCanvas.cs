	...
	public class ShapesCanvas : MonoBehaviour {
		...
		public void InstantiateShapes(){
			...
			for (int i = 0 ; i < ShapesManagerDraw.instance.shapes.Count ;i++){
				if(ShapesManagerDraw.instance.shapes[i] == null){
					continue;
				}
				GameObject shape = Instantiate (ShapesManagerDraw.instance.shapes[i].gamePrefab, Vector3.zero, Quaternion.identity) as GameObject;
				shape.name  = ShapesManagerDraw.instance.shapes[i].gamePrefab.name;//set the name of the shape
				...
				ShapesManagerDraw.instance.shapes[i].gamePrefab = shape.gameObject;
			}
		}
	}
}
