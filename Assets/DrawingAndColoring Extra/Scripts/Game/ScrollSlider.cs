...
namespace IndieStudio.DrawingAndColoring.Logic
{
	...
	{	
		...
		public void Init(){
			...
			pointers = CommonUtilDraw.FindGameObjectsOfTag ("Pointer");
			groups = CommonUtilDraw.FindGameObjectsOfTag ("Group");
		...
		private void DisableFarGroups (int groupIndex)
		{
			...
			for (int i = 0; i < groups.Length; i++) {
				if (i == groupIndex - 1 || i == groupIndex || i == groupIndex + 1) {
					CommonUtilDraw.EnableChildern(groups [i].transform);
				} else {
					CommonUtilDraw.DisableChildern(groups [i].transform);
				}
			}
		}
	}
