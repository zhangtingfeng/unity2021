...
#pragma warning disable 0219 // variable assigned but not used.
	...
	public class ShapesTableLetter : MonoBehaviour
	{
		/// <summary>
		...
		private IEnumerator CreateShapes()
		{
			...
			for (int i = 0; i < shapesManager.shapes.Count; i++)
			{
				...
				for (int s = 0; s < shapeComponents.Count; s++)
				{
					...
					int from, to;
					string[] slices;
					List<Transform> paths = CommonUtilLetter.FindChildrenByTag(shapeComponents[s].transform.Find("Paths"), "Path");
					foreach (Transform p in paths)
					{
						...
						Image img = CommonUtilLetter.FindChildByTag(p, "Fill").GetComponent<Image>();

						if (PlayerPrefs.HasKey(DataManager.GetPathStrKey(ID, compoundID, from, to, shapesManager)))
						{

							List<Transform> numbers = CommonUtilLetter.FindChildrenByTag(p.transform.Find("Numbers"), "Number");
		...
		private void SettingUpShape(TableShape tableShape, int ID, int groupIndex)
		{
			...
			if (tableShape.starsNumber == TableShape.StarsNumber.ONE)
			{//One Star
			...
			else if (tableShape.starsNumber == TableShape.StarsNumber.TWO)
			{//Two Stars
				tempTransform.Find("FirstStar").GetComponent<Image>().sprite = starOn;
			...
			else if (tableShape.starsNumber == TableShape.StarsNumber.THREE)
			{//Three Stars
				tempTransform.Find("FirstStar").GetComponent<Image>().sprite = starOn;
				tempTransform.Find("SecondStar").GetComponent<Image>().sprite = starOn;
