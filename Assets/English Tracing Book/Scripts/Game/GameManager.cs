...
namespace IndieStudio.LetterWrite.Utility
{
	...
	public class GameManager : MonoBehaviour
	{
		...
		/// </summary>
		private ShapesManagerLetter shapesManager;
		...
		void Awake()
		{
			...
			{
				shapesManager = GameObject.Find(ShapesManagerLetter.shapesManagerReference).GetComponent<ShapesManagerLetter>();
			}
			else
			{
		...
		private void OnStartHitCollider(RaycastHit2D hit2d)
		{
			...
			pathFillImage = CommonUtilLetter.FindChildByTag(path.transform, "Fill").GetComponent<Image>();
			...
			else
			{
				...
				CommonUtilLetter.FindChildByTag(path.transform, "Fill").GetComponent<Image>().color = currentPencil.value;
			}
		...
		{
			if (TableShape.selectedShape.ID >= 1 && TableShape.selectedShape.ID < ShapesTableLetter.shapes.Count)
			{
				...
				if (TableShape.selectedShape.ID + 1 <= shapesManager.shapes.Count)
				{
					...
					if (DataManager.IsShapeLocked(TableShape.selectedShape.ID + 1, shapesManager))
					{
						...
						{
							CommonUtilLetter.PlayOneShotClipAt(lockedSFX, Vector3.zero, effectsAudioSource.volume);
						}
						//Skip the next
						return;
					}
				}
				TableShape.selectedShape = ShapesTableLetter.shapes[TableShape.selectedShape.ID];//Set the selected shape
				CreateShape();//Create new shape

			}
			else
			{
				if (TableShape.selectedShape.ID == ShapesTableLetter.shapes.Count)
				{
		...
		public void PreviousShape()
		{
			if (TableShape.selectedShape.ID > 1 && TableShape.selectedShape.ID <= ShapesTableLetter.shapes.Count)
			{
				TableShape.selectedShape = ShapesTableLetter.shapes[TableShape.selectedShape.ID - 2];
				CreateShape();
			}
			else
			{
		...
		private void CreateShape()
		{
			...
			Transform restConfirmMessage = CommonUtilLetter.FindChildByTag(GameObject.Find("ResetConfirmDialog").transform, "Message");
		...
		private void LinearFill()
		{
			...
			Rect rect = CommonUtilLetter.RectTransformToScreenSpace(path.GetComponent<RectTransform>());
		...
		private void OnShapeComplete()
		{
			...
			if (allDone)
			{
				...
				{
					CommonUtilLetter.PlayOneShotClipAt(completedSFX, Vector3.zero, effectsAudioSource.volume);
				}
				AdsManagerLetter.instance.HideAdvertisment();
				AdsManagerLetter.instance.ShowAdvertisment(AdPackage.AdEvent.Event.ON_SHOW_WIN_DIALOG);
			}
			else
			{
				PlayCorrectSFX();
		...
		private void SaveShapeStatus(List<Shape> shapes)
		{
			...
			DataManager.SaveShapeStars(TableShape.selectedShape.ID, CommonUtilLetter.GetTableShapeStars(GameObject.FindObjectOfType<Progress>().starsNumber), shapesManager);
			if (TableShape.selectedShape.ID + 1 <= shapesManager.shapes.Count)
			{
			...
			foreach (Shape s in shapes)
			{
				...
				List<Transform> paths = CommonUtilLetter.FindChildrenByTag(s.transform.Find("Paths"), "Path");
				int from, to;
				string[] slices;
				foreach (Transform p in paths)
				{
					...
					DataManager.SaveShapePathColor(TableShape.selectedShape.ID, compundID, from, to, CommonUtilLetter.FindChildByTag(p, "Fill").GetComponent<Image>().color, shapesManager);
				}
			}
		}
		...
		public void PlayCorrectSFX()
		{
			...
			{
				CommonUtilLetter.PlayOneShotClipAt(correctSFX, Vector3.zero, effectsAudioSource.volume);
			}
		}
