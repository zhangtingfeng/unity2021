...
namespace IndieStudio.LetterWrite.Utility
{
	...
	public class ScrollSlider : MonoBehaviour
	{
		...
		public void Init()
		{
			...
			pointers = CommonUtilLetter.FindGameObjectsOfTag("Pointer");
			groups = CommonUtilLetter.FindGameObjectsOfTag("Group");
		...
		private void DisableFarGroups(int groupIndex)
		{
			...
			for (int i = 0; i < groups.Length; i++)
			{
				...
				{
					CommonUtilLetter.EnableChildern(groups[i].transform);
				}
				else
				{
					CommonUtilLetter.DisableChildern(groups[i].transform);
				}
			}
		}
	}
