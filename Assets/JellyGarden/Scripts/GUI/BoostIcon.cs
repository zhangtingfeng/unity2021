...
public class BoostIcon : MonoBehaviour
{
	...
	void OnEnable ()
	{
		if (name != "Main Camera") {
			if (LevelManager.THIS != null) {
				if (LevelManager.THIS.gameStatus == GameStateGarden.Map)
	...
	public void ActivateBoost ()
	{
		...
		if (IsLocked () || check || (LevelManager.THIS.gameStatus != GameStateGarden.Playing && LevelManager.THIS.gameStatus != GameStateGarden.Map))
			return;
		if (BoostCount () > 0) {
