...
public class LevelMakerEditor : EditorWindow {
	...
	void OnFocus () {
		...
		if (EditorSceneManager.GetActiveScene ().name == "gameJellyGarden") {
...
//		}
		...
		foreach (AdEvents item in initscript.adsEvents) {
			...
			item.gameEvent = (GameStateGarden)EditorGUILayout.EnumPopup (item.gameEvent, new GUILayoutOption[] { GUILayout.Width (100) });
