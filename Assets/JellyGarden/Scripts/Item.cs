...
public class Item : MonoBehaviour
{
    ...
    /// 所用的颜色 ztf
    /// </summary>
    private int Corlorsound = -1;/// <summary>
    /// 
    /// </summary>
    ...
    public void GenColor(int exceptColor = -1, bool onlyNONEType = false)
    {
        ...
        {
            for (int i = 0; i < 6; i++) {
                items[i] = Assets.Script.PunPinYin.StaticGlobal.gStaticTrainingobj.ListSprite[i];
                items[i].name = "item_01" + (i + 1).toString();
    ...
    public void SetColor(int col)
    {
        ...
        if (color < items.Length)
        {
            ...
            //if (Assets.Script.PunPinYin.StaticGlobal.gStaticTrainingobj != null)
            //{
            //    sprRenderer.sprite = Assets.Script.PunPinYin.StaticGlobal.gStaticTrainingobj.ListSprite[color];
            //}
            //else {
        ...
        if (switchDirection != Vector3.zero && neighborSquare != null)
        {
            ...
            else
            {
                if (Corlorsound > -1) {
                    if (Assets.Script.PunPinYin.StaticGlobal.gStaticTrainingobj == null)
                    {
                        SoundBase.Instance.GetComponent<AudioSource>().PlayOneShot(SoundLists[Corlorsound]);///ztf
                    }
                    else {
                        GameObject myCamera = GameObject.Find("Main Camera");
                        myCamera.SendMessage("PlayLocalFile", Assets.Script.PunPinYin.StaticGlobal.gStaticTrainingobj.ListSpriteSound[Corlorsound]);
                    }
                    /// SoundBase.Instance.GetComponent<AudioSource>().PlayOneShot(SoundLists[Corlorsound]);///ztf
                }
    ...
    IEnumerator FallingCor(Square _square, bool animate)
    {
        ...
        Vector3 startPos = transform.position;
        float speed = 10;
        if (LevelManager.THIS.gameStatus == GameStateGarden.PreWinAnimations)
            speed = 10;
    ...
    IEnumerator DestroyCor(bool showScore = false, string anim_name = "", bool explEffect = false)
    {
        ...
        if (currentType == ItemsTypes.HORIZONTAL_STRIPPED)
            ...
            DestroyPackage();
        else if (currentType == ItemsTypes.BOMB && LevelManager.THIS.gameStatus == GameStateGarden.PreWinAnimations)
