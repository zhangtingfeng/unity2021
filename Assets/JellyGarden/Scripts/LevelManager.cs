...
public class LevelManager : MonoBehaviour
{
    ...
    public BoostIcon ActivatedBoost
    {
        ...
        set
        {
            if (value == null)
            {
                if (activatedBoost != null && gameStatus == GameStateGarden.Playing)
    ...
    private GameStateGarden GameStatus;
    ...
    public delegate void GameStateGardenEvents();

    public static event GameStateGardenEvents OnMapState;
    public static event GameStateGardenEvents OnEnterGame;
    public static event GameStateGardenEvents OnLevelLoaded;
    public static event GameStateGardenEvents OnMenuPlay;
    public static event GameStateGardenEvents OnMenuComplete;
    public static event GameStateGardenEvents OnStartPlay;
    public static event GameStateGardenEvents OnWin;
    public static event GameStateGardenEvents OnLose;

    public GameStateGarden gameStatus
    {
        get
        {
            return GameStatus;
        ...
        set
        {
            ...
            if (value == GameStateGarden.PrepareGame)
            {
                passLevelCounter++;
            ...
            else if (value == GameStateGarden.WaitForPopup)
            {
            ...
            else if (value == GameStateGarden.Map)
            {
                ...
                {
                    LevelManager.THIS.gameStatus = GameStateGarden.PrepareGame;
            ...
            else if (value == GameStateGarden.Pause)
            {
                Time.timeScale = 0;

            }
            else if (value == GameStateGarden.Playing)
            {
                Time.timeScale = 1;
                StartCoroutine(AI.THIS.CheckPossibleCombines());
            }
            else if (value == GameStateGarden.GameOver)
            {
                GameObject.Find("CanvasGlobal").transform.Find("MenuFailed").gameObject.SetActive(true);
                OnLose();
            }
            else if (value == GameStateGarden.PreWinAnimations)
            {
                StartCoroutine(PreWinAnimationsCor());
            }
            else if (value == GameStateGarden.Win)
            {
    ...
    public void EnableMap(bool enable)
    {
        ...
        else
        {
            ...
            ///maxRows=5  11
            //GetComponent<Camera>().orthographicSize = 6.5f; ztf
            //GetComponent<Camera>().orthographicSize = 4.9f;
            // GetComponent<Camera>().orthographicSize = 2.9f;

            ///y=fAAA*x+fBBB     when x=5 y=2.9f when x=11 y=5.0f
            float fAAA = (5.0f - 2.9f) / (11f - 5f);

            float fBBB = 2.9f - fAAA * 5f;

            float ffmaxRowsfff = fAAA * maxRows + fBBB;
            GetComponent<Camera>().orthographicSize = ffmaxRowsfff;
    ...
    void Start()
    {
        ...
        gameStatus = GameStateGarden.Map;
        for (int i = 0; i < 20; i++)
        {
    ...
    void PrepareGame()
    {
        ...
        // new Vector2(pos.x + 15, pos.y + yOffset)
        GameField.transform.position = Vector3.zero;
        //   GameField.transform.position = Vector3.zero + new Vector3(-1.5f, 0.6f, 0);//ztf
    ...
    IEnumerator StartAnimateIngredient(GameObject item, int i)
    {
        ...
        Destroy(item);
        if (gameStatus == GameStateGarden.Playing && !IsIngredientFalling())//1.6.1
            CheckWinLose();
        ingredientFly = false;
    }
    ...
    IEnumerator PreWinAnimationsCor()
    {
        ...
        while (GetAllExtaItems().Count > 0 && gameStatus != GameStateGarden.Win)
        { //1.6
        ...
        gameStatus = GameStateGarden.Win;
    }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyUp(KeyCode.Escape))
        {
            ...
            else if (LevelManager.THIS.gameStatus == GameStateGarden.Map)
                Application.Quit();
        ...
        if (LevelManager.THIS.gameStatus == GameStateGarden.Playing)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (hit != null)
                {
                    ...
                    if (!LevelManager.THIS.DragBlocked && LevelManager.THIS.gameStatus == GameStateGarden.Playing)
                    {
    ...
    IEnumerator TimeTick()
    {
        while (true)
        {
            if (gameStatus == GameStateGarden.Playing)
            {
            ...
            if (gameStatus == GameStateGarden.Map)
                yield break;
            yield return new WaitForSeconds(1);
        }
    }
    ...
    IEnumerator NoMatchesCor()
    {
        if (gameStatus == GameStateGarden.Playing)
        {
            ...
            gameStatus = GameStateGarden.RegenLevel;
            yield return new WaitForSeconds(1);
            ReGenLevel();
        }
    }
    ...
    public void ReGenLevel()
    {
        itemsHided = false;
        DragBlocked = true;
        if (gameStatus != GameStateGarden.Playing && gameStatus != GameStateGarden.RegenLevel)
            DestroyItems();
        else if (gameStatus == GameStateGarden.RegenLevel)
            DestroyItems(true);
        OnLevelLoaded();
    ...
    {
        if (gameStatus == GameStateGarden.RegenLevel)
        {
            //while (!itemsHided)
            //{
            yield return new WaitForSeconds(0.5f);
        ...
        if (gameStatus == GameStateGarden.RegenLevel)
            gameStatus = GameStateGarden.Playing;
    ...
    IEnumerator FallingDown()
    {
        ...
        if (gameStatus == GameStateGarden.Playing && !ingredientFly)
            LevelManager.THIS.CheckWinLose();

        if (combo > 2 && gameStatus == GameStateGarden.Playing)
        {
    ...
    {//1.6.1
        if (gameStatus == GameStateGarden.PreWinAnimations)
            return true;
        GameObject[] items = GameObject.FindGameObjectsWithTag("Item");
        foreach (GameObject item in items)
        {
    ...
    void UpdateBar()
    {
        float UpdateDisplay = 0f;
        try
        {
            UpdateDisplay = (float)Score * 100f / ((float)star1 / ((star1 * 100f / star3)) * 100f) / 100f;
        }
        catch (Exception eee)
        {
            Debug_Log.Call_WriteLog(eee, "unityError");
        }
        if (ProgressBarScript.Instance)
        {
            ProgressBarScript.Instance.UpdateDisplay(UpdateDisplay);
        }
